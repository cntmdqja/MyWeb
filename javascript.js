...
$(document).ready(function() {$('#ClassCoor').click(function (e) {
...
function alertDiv(text, left, top) {
	...
	else if(text == 'ClassCoor'){
		var start = '<div class="alert">';
		var title = '<h1>Class Coordinator</h1>';
		var language = '<h5>Language used: </h5> <p>Java, JSP, HTML, CSS, SQL Javascript, JQuery</p>';
		var purpose = '<h5>Purpose: </h5><p>ClassCoordinator is a JSP based website that will enable users to keep track of each other. This website is for those who are working in group, mainly in college or high school </p>';
		var whatIDid = '<h5>Contribution: </h5><p>CSS styling, html format, Javascript, SQL queries, Java classes and servlets used,</p>'
		var URL = '<a href="https://github.com/cntmdqja/ClassCoor">Go to the github page</a>';
		var end = '</div>';
	}
    ...
    $("#close").click(function () {
        ...
        alrt.remove();
		$('#screen').css({"display": "none", opacity: 0.7, "width":$(document).width(),"height":$(document).height()});
    });
...
$(document.querySelectorAll("#history, #partnerdays, #Agreement, #ispmain, #CodeLoad, #ClassCoor")).click(pop);
$(window).resize(function(){
        $('.alert').css("display") == 'block'?pop.call($(querySelectorAll("#ClassCoor,#history, #partnerdays, #Agreement, #ispmain, #CodeLoad"))):"";
});
});
